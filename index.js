...
const notifyOfExamResults = () => {
  const students = [
    ...scores
      .reduce(
        (map, student) =>
          map.has(student.name) ? map : map.set(student.name, student),
        new Map()
      )
      .values()
  ];
...
const hasPassed = (name, score) => {
  return score > 50
    ? sendPassedEmail(name, score)
    : sendFailedEmail(name, score);
};

const sendPassedEmail = (name, score) => {
  let email = findStudentEmail(name);
  let emailSentSuccessfully = sendEmail(
    email,
    successMessage(capitaliseName(name), score)
  );
...
const sendFailedEmail = (name, score) => {
  ...
  let emailSentSuccessfully = sendEmail(
    email,
    failureMessage(capitaliseName(name), score)
  );
  if (!emailSentSuccessfully) return didNotSend.push(email);
};
